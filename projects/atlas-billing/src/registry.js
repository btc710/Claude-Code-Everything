...
// Mirror of atlas-model-gateway's slot registry. These are COGS — the per-1M
// vendor prices the platform pays. Tenant-facing unit prices are derived in
// `pricing.js` (cogs * margin_multiplier, or a per-tenant override).
//
// Keep this list in sync with `projects/model-gateway/src/registry.js`.
// In production, both services would read from a shared pricing service /
// versioned config; here we duplicate to keep atlas-billing self-contained.
const REGISTRY = [
